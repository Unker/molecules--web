...
System.out.println ("MKP " + this.activeMenu + " " + name + " / " + text);
if (this.activeMenu === "bondMenu") {
this.bondHoverLabel = text;
} else if (this.activeMenu === "atomMenu") this.atomHoverLabel = text;
...
var autoBond = (data[1] >= 0);
var addHsAndBond = (data[2] >= 0);
this.assignAtom (atomIndex, type, autoBond, addHsAndBond, true);
return null;
...
this.appRunScript ("modelkit connect " + mp.getMeasurementScript (" ", false) + " true");
...
this.vwr.sm.modifySend (atomIndex, this.vwr.ms.at[atomIndex].mi, 1, cmd);
this.assignAtom (atomIndex, type, true, true, true);
...
if (!isMK) this.vwr.setBooleanProperty ("modelkitmode", true);
bs = this.vwr.addHydrogensInline (bs, vConnections, pts);
if (!isMK) this.vwr.setBooleanProperty ("modelkitmode", false);
...
function (index, index2, cmd, isClick) {
...
var ac = this.vwr.ms.ac;
this.assignAtom (index, ".", true, true, isClick);
this.assignAtom (index2, ".", true, true, isClick);
this.vwr.ms.setAtomNamesAndNumbers (0, -ac, null);
